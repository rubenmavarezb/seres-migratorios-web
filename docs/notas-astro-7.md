...
- **`site` en `astro.config.mjs`** tiene que ser una URL válida: `https://[DOMINIO]` rompe el build. Se lee de `PUBLIC_SITE_URL` (con `loadEnv` de `vite`) y cae al dominio oficial `https://seresmigratorios.com` (SM-010 y SM-064 hechas el 06.09.2026; la URL interna `seres-migratorios.netlify.app` no se usa en `site` ni en canonical). `PUBLIC_SITE_URL` está cargada en el panel de Netlify con ese mismo valor.
